...
def scale_from_float_to_int64(scale):
    """Convert float32 scale to int64 representation for fused MC2."""
    import numpy as np
...
class AscendW8A16FP8LinearMethod(AscendLinearScheme):
    """Linear method for Ascend W8A16_FP8.

    This scheme uses FP8 (float8_e4m3fn) quantized weights with 16-bit
    activations. Since ``npu_weight_quant_batchmatmul`` does not support FP8
    weights, activations are dynamically quantized to FP8 at runtime and
    ``npu_quant_matmul`` is used for the matmul operation.

    Operator selection rationale:
    - ``npu_dynamic_quant(dst_type=float8_e4m3fn)``: Per-token dynamic
      quantization of 16-bit activations to FP8 E4M3FN format.
    - ``npu_quant_matmul``: Chosen over ``npu_weight_quant_batchmatmul``
      because the latter does not support FP8 weight inputs. Supports
      per-token activation scales and per-channel weight scales.
    - Bias is added after matmul (not fused) due to a known NPU operator
      issue with FP8 bias fusion in ``npu_quant_matmul``.
    """
    ...
    def apply(
        ...
        quantized_x, pertoken_scale = torch_npu.npu_dynamic_quant(x, dst_type=torch.float8_e4m3fn)
        need_unsqz = False
...
class AscendW8A16FP8FusedMoEMethod(AscendMoEScheme):
    """FusedMoE method for Ascend W8A16_FP8.

    This scheme uses FP8 (float8_e4m3fn) quantized weights with 16-bit
    activations for MoE layers. Weights are dequantized on-the-fly using
    per-channel scales and zero offsets during the grouped matmul operation.

    Operator selection rationale:
    - MoE dispatch uses ``fused_experts`` with ``QuantType.W8A16FP8``.
    - Weight scales are stored as float32 for maximum precision during
      dequantization.
    - Supports multistream_overlap_gate, zero_experts, dynamic_eplb,
      and fused_mc2 for performance optimization.
    """
    ...
    def get_dynamic_quant_param(
        ...
        param_dict["w2_weight_offset"] = torch.zeros(num_experts, hidden_sizes, 1, dtype=params_dtype)
        return param_dict
    ...
    def apply(
        ...
        if zero_expert_num == 0 or zero_expert_type is None:
            assert router_logits.shape[1] == num_logical_experts, (
                "[vllm-ascend/W8A16_FP8] Number of global experts mismatch "
        ...
        if self.multistream_overlap_gate:
            ...
            assert fc3_context is not None, (
                "[vllm-ascend/W8A16_FP8] flash_common3 context is required "
